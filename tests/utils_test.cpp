...
TEST(utils_test, trim) {
...

  {
    ...
    EXPECT_EQ(config::utils::trim(test_str), base);
  }
  {
    // Trailing whitespace
    std::string test_str = base + "   \n\n   ";
...
TEST(utils_test, split) {
  ...
  const std::vector<std::string> parts = {"this", "is", "a", "test"};
  {
    ...
    const auto split = config::utils::split(combined, '.');
...
TEST(utils_test, join) {
  {
    ...
    const auto output = config::utils::join(input, ".");
  ...
  }
  {
    ...
    const auto output = config::utils::join({}, ";");
...
TEST(utils_test, splitAndJoin) {
  {
    ...
    const auto joined = config::utils::join(input, ".");
    const auto split = config::utils::split(joined, '.');
...
TEST(utils_test, makeName) {
  {
    ...
    const auto result = config::utils::makeName(expected, "");
  ...
  }
  {
    ...
    const auto result = config::utils::makeName(part1, part2);
    EXPECT_EQ(expected, result);
  }
}
